    ...
    def execute_code(self, code: str, max_attempts: int = 3) -> Any:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # before exec(...)
                print("==== START: code_str (len={} bytes) ====\n".format(len(code_str)))
                print(code_str)
                print("==== END: code_str ====")

                # try to compile first to get useful syntax error
                try:
                    compiled = compile(code_str, "<string>", "exec")
                except SyntaxError as e:
                    print("SYNTAX ERROR detected during compile():")
                    traceback.print_exc()
                    # return JSON response or raise with details
                    raise RuntimeError(f"Syntax error in generated code: {e.msg} at line {e.lineno}: {e.text}")

                # if compile OK, then exec
                exec_globals = {}
